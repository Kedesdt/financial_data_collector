...
from vmix_updater.updater import ApiUpdater
...
class FinancialDataCollector:
    ...
    def collect_all_data(self) -> Dict:
        ...
        try:
            ...
            if self.debug:
                print(
                    f"✅ Coletados {len(cambio_data)} pares de moedas e {len(bolsa_data)} índices"
                )
    ...
    def format_bloomberg_style(self, data: Dict) -> str:
        ...
        if "market_status" in data and data["market_status"]:
            ...
            output.append(
                f"🏛️  B3: {status_icon} {'ABERTO' if status['is_open'] else 'FECHADO'}"
            )
            output.append("")
        ...
        if "cambio" in data:
            for pair, info in data["cambio"].items():
                ...
                output.append(
                    f"{pair:12} {info['rate']:8.4f} {trend} {info['change_percent']:+6.2f}%"
                )
        ...
        if "bolsa" in data:
            for index, info in data["bolsa"].items():
                trend = "📈" if info["change"] >= 0 else "📉"
                output.append(
                    f"{index:12} {info['price']:10,.2f} {trend} {info['change_percent']:+6.2f}%"
                )
                output.append(
                    f"             Variação: {info['change']:+,.2f} | Volume: {info['volume']:,}"
                )
    ...
    def get_summary(self, data: Dict) -> Dict:
        ...
        if "market_status" in data:
            summary["market_open"] = (
                data["market_status"].get("B3", {}).get("is_open", False)
            )
